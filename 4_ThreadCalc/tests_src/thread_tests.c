 ...
 * another thread
 */
static int check_unique_thread_id(pthread_t thread_ids[], size_t num_threads);
...
void * check_thread_working(void * arg_p)
{
    if (NULL == arg_p)
    {
        return NULL;
...
int check_unique_thread_id(pthread_t thread_ids[], size_t num_threads)
{
    ...
    for (size_t i = 0; i < num_threads; i++)
    {
        ...
        {
            id = pthread_equal(thread_ids[i], thread_ids[j]);
            if (0 != id)
            {
                all_unique = ERROR;
                goto END;
    ...
    for (size_t i = 0; i < num_threads; i++)
    {
        if (pthread_equal(thread_ids[i], thread_ids[num_threads]))
        {
            all_unique = i;
            if (0 == i)
            {
